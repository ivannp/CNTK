...
public:
    ...
    static size_t FindBraces(const std::string& str, const size_t tokenStart)
    {
        ...
        for (auto current = tokenStart; current + 1 < len;)
        {
            ...
            {
                const auto bracePos = openBraces.find(brace);  // index of brace
                braceStack.push_back(closingBraces[bracePos]); // closing symbol for current
            }
        }
