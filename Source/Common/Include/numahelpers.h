...
static inline void *malloc(size_t n, size_t align)
{
    ...
    if (p == NULL)
        fprintf(stderr, "numa::malloc: failed allocating %zd bytes with alignment %zd\n", n, align);
    if (((size_t) p) % align != 0)
...
static inline void showavailablememory(const char *what)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        if (rc)
            fprintf(stderr, "%s: %8.2f MB available on NUMA node %zd\n", what, availmb, i);
        else
            fprintf(stderr, "%s: error for getting available memory on NUMA node %zd\n", what, i);
    }
}
