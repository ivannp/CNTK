...
vector<lattice::nodecontext> lattice::determinenodecontexts(const msra::asr::simplesenonehmm &hset) const
{
        ...
        if (Z == spunit)
        {
            ...
            else
            {
                ...
                if (Z == spunit) // a bugg lattice --I got this from HVite, to be tracked down
                {
                    ...
                    fprintf(stderr, "determinenodecontexts: word with %zu /sp/ at the end found, edge %d\n", a.size() - 1 - n, j);
                    multispseen++;
                    Z = a[n].unit;
                }
            }
    ...
    if (multispseen > 0)
        fprintf(stderr, "determinenodecontexts: %zu broken edges in %zu with multiple /sp/ at the end seen\n", multispseen, edges.size());
...
void lattice::merge(const lattice &other, const msra::asr::simplesenonehmm &hset)
{
    ...
    fprintf(stderr, "merge: joint node space uniq'ed to %d from %zu\n", j, contexts.size());
    contexts.resize(j);
...
void lattice::dedup()
{
    ...
    fprintf(stderr, "dedup: edges reduced to %zu from %zu\n", k, edges2.size());
    edges2.resize(k);
                                 ...
                                 const msra::asr::simplesenonehmm &hset)
{
    ...
    if (skippedmerges > 0)
        fprintf(stderr, "convert: %zu out of %zu merge operations skipped due to secondary lattice missing\n", skippedmerges, toclines.size());

    // write out the updated unit map
    if (f && ftoc)
        writeunitmap(symlistpath, modelsymmap);

    fprintf(stderr, "converted %zu lattices\n", toclines.size());
}
...
void lattice::fromhtklattice(const wstring &path, const std::unordered_map<std::string, size_t> &unitmap)
{
    ...
    for (; iter != lines.end() && strncmp(*iter, "N=", 2); iter++)
    {
        if (strncmp(*iter, "lmscale=", 8) == 0) // note: HTK sometimes generates extra garbage space at the end of this line
            if (sscanf_s(*iter, "lmscale=%f wdpenalty=%f%c", &info.lmf, &info.wp, &dummychar, (unsigned)(dummychar)) != 2 && dummychar != ' ')
    ...
    {
        unsigned long N, L;
        if (sscanf_s(*iter, "N=%lu L=%lu %c", &N, &L, &dummychar, (unsigned)sizeof(dummychar)) != 2)
    ...
    for (size_t i = 0; i < info.numnodes; i++, iter++)
    {
        ...
        unsigned long itest;
        float t;
        if (sscanf_s(*iter, "I=%lu t=%f%c", &itest, &t, &dummychar, (unsigned)sizeof(dummychar)) < 2)
    ...
    for (size_t j = 0; j < info.numedges; j++, iter++)
    {
        ...
        int nvals = sscanf_s(*iter, "J=%lu S=%lu E=%lu a=%f l=%f d=%s", &jtest, &S, &E, &a, &l, &d, (unsigned)sizeof(d));
        ...
        {
            char msg[128];
            sprintf(msg, "\n-- where edgeframes=%zu != (nodes[E].t - nodes[S].t=%zu), the gap is %zu.", edgeframes, nodes[E].t - (size_t) nodes[S].t, edgeframes + nodes[S].t - nodes[E].t);
